...
pub fn view() {
  html.p([attribute.class("center"), attribute.style([#("text-align", "justify")])], [
    element.text(
    ...
    html.hr([attribute.style([#("margin-top", "0.5em"), #("margin-bottom", "0.5em")])]),
        html.img([
          attribute.src("
        ...
        element.text(", painting a picture of a Flint resident's embrace of emptiness. Its instrumentation is fittingly sparse, with accompinament mostly coming from piano and heavily reverbed trumpet. The latter instrument builds with repetition of of the lyric "),
        html.q([], [element.text("Even if I died alone")]),
        element.text(" to express resignment to and eventual proclamation of this line."),
      html.br([attribute.style([#("margin-bottom", "0.5em")])]),
        element.text("The subsequent "),
        html.cite([], [element.text("All Good Naysayers, Speak Up! Or Forever Hold Your Peace!")]),
        element.text(" introduces the other aspect to this recurring dialogue with the pathological appeal deficiencies in society as they are ("),
        html.q([], [element.text("Often not the state is advocation/If we forma power of recognition")]),
        element.text(", "),
        html.q([], [element.text("Entertain ideas of great communion/Shelter not materials in union")]),
        element.text("). It makes no call to action beyond acknowledgement, and its innocent motivation is accentuated by the exclamation in the title and the song itself's jaunty 5/4. This pseudo-naïveté continues into the reminiscence and appreciation of "),
        html.cite([], [element.text("Say Yes! to M!ch!gan!")]),
      html.br([attribute.style([#("margin-bottom", "0.5em")])]),
        element.text("Facing the bleakness of reality can lead to the empty despair described in "),
        html.cite([], [element.text("Flint")]),
        element.text(" as exemplified by the end of "),
        html.cite([], [element.text("The Upper Peninsula")]),
        element.text(" with the panicking synth rising in prominence from the line "),
        html.q([], [element.text("I lost my mind, I lost my life/I lost my job, I lost my wife")]),
        element.text(" until suddenly giving way to a gentle guitar chord, as though abruptly resigning."),
      html.br([attribute.style([#("margin-bottom", "0.5em")])]),
        html.cite([], [element.text("Oh Detroit, Lift Up Your Weary Head! (Rebuild! Restore! Reconsider!)")]),
        element.text(" is perhaps the most direct look at reality, taking a more detached and cynical tone in observance of the rein wrought on Detroit through the intense proliferation and stark abandomnemt of industry, returning to the upbeat 5/4 of"),
      ...
      html.br([attribute.style([#("margin-bottom", "0.5em")])]),
        ...
        element.text(" expresses not frustration but rather counsel, expressing hope for God's "),
        html.q([], [element.text("perfect design")]),
        element.text(" before eventually pleading with nature, reflecting on the forgotten pain of the buried Surgeon Bay and the drowned of Saint Marie. "),
      html.br([attribute.style([#("margin-bottom", "0.5em")])]),
        element.text("The proceeding "),
        html.cite([], [element.text("They Also Mourn Who Do Not Wear Black (For the Homeless in Muskegon)")]),
        element.text(
        ...
        html.q([], [element.text("If the advantageous/Reprimand misgivings/We won't grow/We will not ever know")]),
      ...
      html.br([attribute.style([#("margin-bottom", "0.5em")])]),
        html.cite([], [element.text("Oh God Where Are You Now? (In Pickerel Lake? Pigeon? Marquette? Mackinaw?)")]),
        element.text(" nonetheless searches again for divine comfort and judgement while the narrator question's the relationship between their own perception of righteousness and that of God's ("),
        html.q([], [element.text("Oh God, hold me now")]),
        element.text(", "),
        html.q([], [element.text("Would the righteous still remain?/Would my body stay the same?")]),
      ...
      html.br([attribute.style([#("margin-bottom", "0.5em")])]),
        ...
        element.text(" ("),
        html.q([], [element.text("Rest in my arms/Sleep in my bet/There's a design/To what I did and said")]),
        element.text(", quelling the prior doubt by asserting that all is as ordained by univeral, divine love, and serving as a cathartic epilogue of sorts to the album. Repeating this like a matra, this expresses an internal resolution and restored faith, despite it all."),
    html.hr([attribute.style([#("margin-top", "0.5em"), #("margin-bottom", "0.5em")])]),
        html.img([
          attribute.src("
        ...
        element.text(" is not a the canonical sequel to ") ,
        html.cite([], [element.text("Michigan")]),
        element.text(" (that title goes to the seminal 2005 work "),
        html.cite([], [element.text("Illinois")]),
        element.text("), its even more overt Christian themes serve as a fearful yet accepting reflection of the journey of the prior album. "),
        html.cite([], [element.text("All the Trees of the Field Will Clap Their Hands")]),
        ...
        element.text(") but defers to the divine despite expressed concern over whether they are on the right side of the outcome ("),
        html.q([], [element.text("And will I be invited to the sound?/And will I be a part of what you've made?")]),
      ...
      html.br([attribute.style([#("margin-bottom", "0.5em")])]),
        ...
        element.text(" matter-of-factly dubs the world as having already fallen, portending the end of times as something inevitable to be anticipated ("),
        html.q([], [element.text("Be still and know your sign/The beast will arrive in time")]),
        element.text(" yet maintaining a placid tone, seeing this as an opportunity to meet God rather than as a threat ("),
        html.q([], [element.text("We stayed a long, long time/But I'm not afraid to die/To see you/To meet you/To see you/At last")]),
        element.text(") aided by the light banjo, ephemeral vocals, and whistling synths. "),
      html.br([attribute.style([#("margin-bottom", "0.5em")])]),
        element.text("The subsequent few tracks exemplify the overt Christian theming with direct gratitude towards Jesus and Abraham, and "),
        html.cite([], [element.text("We Won't Need Legs To Stand")]),
        element.text(" looks forward to the burdenless afterlife as a final reward for the unchanging present, approaching the subject with a dark yet optimistically resolvent tone. Inversely, "),
        html.cite([], [element.text("A Good Man Is Hard To Find")]),
        element.text(" displays an acceptance of Hell as a result of failing to satisfactorily better oneself after costing another ("),
        html.q([], [element.text("And so I go to Hell/I wait for it but someone's left me creased")]),
        element.text(" in so doing casting doubt on the innocence of all complicit in the system through comparison ("),
        html.q([], [element.text("Once in the backyard/She was once like me")]),
        element.text("). The gravitas of this seeming contradiction is hauntingly emphasized by the detuned vocals present in the bridge."),
      html.br([attribute.style([#("margin-bottom", "0.5em")])]),
        element.text("The title track capitalizes on this by recalling "),
        html.cite([], [element.text("In The Devil's Territory")]),
        element.text(", actually showing the judgement alluded to ("),
        html.q([], [element.text("I saw a sign in the sky/Seven horns, Seven horns, Seven horns")]),
        element.text("). The vocals cycle between rising hopefully and falling back down, quieting to a whisper. After reassurance of God's presence comes a grave proclamation of omnipotence that communicates universal compassion in a manner akin to a threat ("),
        html.q([], [element.text("He will take you/If you run, He will chase you/'Cause He is the Lord")]),
        element.text(" with a correspondingly booming piano part, Sufjan's icy falsetto sitting above the dark, swirling colours evoking fate, finally resolving peacefully."),
      html.br([attribute.style([#("margin-bottom", "0.5em")])]),
        html.cite([], [element.text("The Transfiguration")]),
        element.text(", the final track, is more upbeat (with a melody eventually evoked by "),
        html.cite([], [element.text("Chicago")]),
        element.text(", the final track, is more upbeat, being told from the perspective of a truely benevolent diety tending to humanity as a parent ("),
        html.q([], [element.text("Lost in the cloud, a voice/Lamb of God, we draw near")]),
        element.text("."),
    html.hr([attribute.style([#("margin-top", "0.5em"), #("margin-bottom", "0.5em")])]),
      element.text(
